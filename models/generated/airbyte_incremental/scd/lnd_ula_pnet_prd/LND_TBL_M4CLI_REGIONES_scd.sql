{{ config(
    ...
    post_hook = ["
                    ...
                    set final_table_relation = adapter.get_relation(
                            ...
                            schema=this.schema,
                            identifier='LND_TBL_M4CLI_REGIONES'
                        )
                    %}
                    {#
                    ...
                    delete from {{ final_table_relation }} final_table where final_table._airbyte_unique_key in (
                        ...
                        from (
                                ...
                                from {{ this }}
                                where 1=1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('LND_TBL_M4CLI_REGIONES')) }}
                            ) recent_records
                            left join (
                                select _airbyte_unique_key as unique_key, count(_airbyte_unique_key) as active_count
                                from {{ this }}
                                where _airbyte_active_row = 1 {{ incremental_clause('_airbyte_normalized_at', adapter.quote(this.schema) + '.' + adapter.quote('LND_TBL_M4CLI_REGIONES')) }}
                    ...
                    {% endif %}
                    ","drop view _airbyte_lnd_ula_pnet_prd.LND_TBL_M4CLI_REGIONES_stg",'{{ 
...
-- depends_on: ref('LND_TBL_M4CLI_REGIONES_stg')
with
{% if is_incremental() %}
new_data as (
    ...
    select
        *
    from {{ ref('LND_TBL_M4CLI_REGIONES_stg')  }}
...
previous_active_scd_data as (
    ...
    select
        {{ star_intersect(ref('LND_TBL_M4CLI_REGIONES_stg'), this, from_alias='inc_data', intersect_alias='this_data') }}
...
input_data as (
    select {{ dbt_utils.star(ref('LND_TBL_M4CLI_REGIONES_stg')) }} from new_data
    union all
    select {{ dbt_utils.star(ref('LND_TBL_M4CLI_REGIONES_stg')) }} from previous_active_scd_data
),
{% else %}
input_data as (
    select *
...
scd_data as (
    ...
    select
      ...
      lag(DT_LAST_UPDATE) over (
        partition by cast(CLI_ID_REGION as {{ dbt_utils.type_string() }}), ID_ORGANIZATION
        order by
